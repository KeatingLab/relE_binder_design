...
if __name__ == '__main__':
    ...
    parser.add_argument('--dtermen_type', help='type of dtermen etab scoring performed', default="enerTable")
    ...
    if args.dtermen_type == "design":
        glob_dir =  glob.glob(os.path.join(output_path, '*-output.out'))
    else:
        glob_dir =  glob.glob(os.path.join(output_path, '*.out'))
    for filename in glob_dir:
        print(filename)
        if args.dtermen_type == "design":
            pdb_id = os.path.basename(filename)[:-len('-output.out')]
        else:
            pdb_id = os.path.basename(filename).split('.')[0].split('_')[1]
        print(pdb_id)
        ids += [pdb_id]
        try:
            ...
            with open(filename, 'r') as f:
                if args.dtermen_type == "design":
                    ...
                    if len(lines) < 5:
                        ...
                        continue
                    r_nrg, r_seq = lines[0].split(' ')
                    p_seq = lines[2].split(':')[1].split(' ')[1]
                    p_nrg = lines[3].split(':')[1].split(' ')[1]
                    recov = float(lines[4].split(' ')[4])
                    nrg_u = 0
                    nrg_s = 0
    ...
    for etab_file in glob.glob("*.etab"):
        pdb = etab_file[:-5]
        if not (os.path.exists(f"{pdb}-output.out") or os.path.exists(f"design_{pdb}.out")) and pdb not in errors:
            errors.append(pdb)
