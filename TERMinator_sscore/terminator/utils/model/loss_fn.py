...
def nlpl(etab, E_idx, data, sscore):
...
def nlcpl(etab, E_idx, data, sscore):
    ...
    self_nrgs_jn = torch.gather(self_nrgs_im_expand, 1, E_idx_jn_expand) # b x L x 29 x 21 #seb: the E_s for the other residue (Rj) in every contact
...
def nlcpl_test(etab, E_idx, data, sscore):
    """ Alias of nlcpl_full """
    return nlcpl_full(etab, E_idx, data)


def nlcpl_full(etab, E_idx, data, sscore):
    """ Negative log composite psuedo-likelihood
        Averaged nlcpl per residue, across batches
        p(a_i,m ; a_j,n) =
...
def etab_norm_penalty(etab, E_idx, data, sscore):
...
# pylint: disable=unused-argument
def mindren_etab_norm_penalty(etab, E_idx, data, sscore):
    """ Take the norm of all etabs and scale it by the total number of residues involved """
    seq_lens = data['seq_lens']
    # etab_norm = torch.linalg.norm(etab.view([-1]))
    # return etab_norm / seq_lens.sum(), int(seq_lens.sum())
...
# pylint: disable=unused-argument
def etab_l1_norm_penalty(etab, E_idx, data, sscore):
    """ Take the norm of all etabs and scale it by the total number of residues involved """
    seq_lens = data['seq_lens']
    # etab_norm = torch.linalg.norm(etab.view([-1]))
    # return etab_norm / seq_lens.sum(), int(seq_lens.sum())
...
# pylint: disable=unused-argument
def pair_self_energy_ratio(etab, E_idx, data, sscore):
...
def sortcery_loss(etab, E_idx, data, sscore):
...
def structure_score_loss(etab, E_idx, data, sscore):
    total_mse_loss = None
    for i,sscore_target in enumerate(data["sscore"]):
        # select the positions for which there is coordinate info
        masked_score = torch.masked_select(sscore[i],data['x_mask'][i].bool())
        masked_target_score = torch.masked_select(sscore_target,data['x_mask'][i].bool())
        if total_mse_loss == None:
            total_mse_loss = F.mse_loss(masked_score,masked_target_score)
        else:
            total_mse_loss += F.mse_loss(masked_score,masked_target_score)
    total_mse_loss = total_mse_loss / len(data["sscore"])
    return total_mse_loss, int(data['seq_lens'].sum())
