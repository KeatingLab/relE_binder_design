...
class TERMinator(nn.Module):
    ...
    def __init__(self, hparams, device='cuda:0'):
        ...
        if self.hparams['use_flex'] and self.hparams['flex_type'].find("dynamic_signatures") > -1:
            self.hparams['flex_input_dim'] = self.hparams['flex_hidden_dim']
            self.bot = CondenseFlex(hparams, device=self.dev)
        elif self.hparams['use_flex'] and self.hparams['flex_type'].find("conformational_pos_variation") > -1:
            self.hparams['flex_input_dim'] = 1
        else:
            self.hparams['flex_input_dim'] = 0
        ...
        if self.hparams['use_terms']:
            ...
            print(f'Flex information hidden dimensionality is {self.bot.hparams["flex_hidden_dim"]}')
    ...
    def forward(self, data, max_seq_len):
        ...
        data : dict
            ...
            chain_idx : torch.LongTensor
                Integers indices that designate every residue to a chain.
                Shape: n_batch x n_res
        ...
        sscore : torch.Tensor
            ...
            Shape: n_batch x n_res
        """
        if self.hparams['use_terms'] or (self.hparams['use_flex'] and self.hparams['flex_type'].find("dynamic_signatures") > -1):
            node_embeddings, edge_embeddings = self.bot(data, max_seq_len)
        else:
            node_embeddings, edge_embeddings = None, None
        if self.hparams['flex_type'].find('conformational_pos_variation') > -1:
