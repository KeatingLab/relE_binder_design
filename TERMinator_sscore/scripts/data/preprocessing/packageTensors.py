...
def dumpTrainingTensors(in_path, out_path=None, cutoff=1000, save=True, coords_only=False, dummy_terms=None):
    ...
    coords, _, res_info = parseCoords(in_path + '.red.pdb', save=False)
    ...
    if len(coords) == 1:
        ...
        # chains = sorted(coords.keys()) # should already be sorted 
        coords_tensor = np.vstack([coords[c] for c in coords])
...
def dumpCoordsTensors(in_path, out_path=None, save=True, valid_pdb_lines=None):
    ...
    save : bool, default=True
        Whether or not to save the training tensors
    valid_pdb_lines: list, default=None
        If the PDB lines have already been read from the file, pass directly to parseCoords
    ...
    """
    if valid_pdb_lines:
        coords, seq, res_info = parseCoords(None, save=False, valid_entry_lines=valid_pdb_lines, verbose=False)
    else:
    ...
    chain_lens = [len(coords[c]) for c in coords]
    pdb = os.path.basename(in_path) if in_path else ""
...
def concatFeatureDicts(feat_A,feat_B):
    """Combine two existing feature dictionaries (assume that they correspond to distinct chains)
    note: preserves the order in which feature dictionaries are provided, which may not match how they would be ordered if they were created directly by other methods, e.g. dumpCoordsTensor
    ...
    dict
        Dictionary of features for TERMinator
    """
    concat_features = dict()
    if feat_A.keys() != feat_B.keys():
        assert ValueError('Attempting to concatenate two feature dictionaries with different numbers of feature types')
    
    # Create dummy arrays in case they need to copied forward
    dummy_arr_3d = np.zeros([1, 1, 1])
    dummy_arr_2d = np.zeros([1, 1])
    dummy_arr_1d = np.ones([1])
    for feature_type in feat_A:
        if feature_type == 'pdb':
            concat_features['pdb'] = feat_A['pdb'] + '_' + feat_B['pdb'] #combine names
        elif feature_type == 'coords':
            concat_features['coords'] = np.concatenate((feat_A['coords'],feat_B['coords']),axis=0)

        # Begin dummy feature section
        elif feature_type == 'ppoe':
            if feat_A['ppoe'] == dummy_arr_3d:
                concat_features['ppoe'] = dummy_arr_3d
            else:
                concat_features['ppoe'] = np.concatenate((feat_A['ppoe'],feat_B['ppoe']),axis=0)
        elif feature_type == 'features':
            if feat_A['features'] == dummy_arr_3d:
                concat_features['features'] = dummy_arr_3d
            else:
                concat_features['features'] = np.concatenate((feat_A['features'],feat_B['features']),axis=0)
        elif feature_type == 'msas':
            if feat_A['msas'] == dummy_arr_2d:
                concat_features['msas'] = dummy_arr_2d
            else:
                concat_features['msas'] = np.concatenate((feat_A['msas'],feat_B['msas']),axis=0)
        elif feature_type == 'focuses':
            if feat_A['focuses'] == dummy_arr_2d:
                concat_features['focuses'] = dummy_arr_2d
            else:
                concat_features['focuses'] = np.concatenate((feat_A['focuses'],feat_B['focuses']),axis=0)
        elif feature_type == 'contact_idxs':
            if feat_A['contact_idxs'] == dummy_arr_2d:
                concat_features['contact_idxs'] = dummy_arr_2d
            else:
                concat_features['contact_idxs'] = np.concatenate((feat_A['contact_idxs'],feat_B['contact_idxs']),axis=0)
        elif feature_type == 'term_lens':
            if feat_A['term_lens'] == dummy_arr_1d.astype(int):
                concat_features['term_lens'] = dummy_arr_1d.astype(int)
            else:
                concat_features['term_lens'] = np.concatenate((feat_A['term_lens'],feat_B['term_lens']),axis=0)
        # End dummy feature section
        
        elif feature_type == 'sequence':
            concat_features['sequence'] = np.concatenate((feat_A['sequence'],feat_B['sequence']),axis=0)
        elif feature_type == 'seq_len':
            concat_features['seq_len'] = feat_A['seq_len'] + feat_B['seq_len']
        elif feature_type == 'chain_ids':
            concat_features['chain_ids'] = feat_A['chain_ids'] + feat_B['chain_ids']
        elif feature_type == 'chain_lens':
            concat_features['chain_lens'] = feat_A['chain_lens'] + feat_B['chain_lens']
        elif feature_type == 'res_info':
            concat_features['res_info'] = feat_A['res_info'] + feat_B['res_info']
