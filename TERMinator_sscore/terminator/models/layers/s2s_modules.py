...
class NodeMPNNLayer(nn.Module):
    def __init__(self, num_hidden, num_in, dropout=0.1, num_heads=None, scale=30, num_ensembles=1):
        super().__init__()
        del num_heads
        self.num_hidden = num_hidden
        self.num_in = num_in
    ...
    def forward(self, h_V, h_E, mask_V=None, mask_attend=None, multiple_ensembles=False, k_neighbors=30, multiple_ensembles_idx = 0):
        """ Parallel computation of full transformer layer """

        # Concatenate h_V_i to h_E_ij
        h_EV = torch.cat([h_V, h_E], -1)

        h_message = self.W3(F.relu(self.W2(F.relu(self.W1(h_EV)))))
        if mask_attend is not None:
            if multiple_ensembles:
                mask_attend = mask_attend.repeat(1, 1, self.num_ensembles)
            h_message = mask_attend.unsqueeze(-1) * h_message ## Is this working as indended? JFM
        dh = h_message
        if multiple_ensembles:
            dh_list = torch.split(dh, k_neighbors, dim=-2)
            h_V_list = torch.split(h_V, k_neighbors, dim=-2)
        else:
            dh_list = [dh]
            h_V_list = [h_V]
        h_V_list_final = []
        if mask_V is not None:
            mask_V = mask_V.unsqueeze(-1)
        for (dh, h_V) in zip(dh_list, h_V_list):
            h_V = torch.mean(h_V, dim=2)
            if (self.scale != k_neighbors):
                dh = torch.sum(dh, -2) / k_neighbors
            else:
                dh = torch.sum(dh, -2) / self.scale
            h_V = self.norm[0](h_V + self.dropout(dh))
            ...
            if mask_V is not None:
                h_V = mask_V * h_V
            if multiple_ensembles:
                h_V = torch.unsqueeze(h_V, 2)
            h_V_list_final.append(h_V)
        h_V = torch.cat(h_V_list_final, dim=-2)
        return h_V


class EdgeMPNNLayer(nn.Module):
    def __init__(self, num_hidden, num_in, dropout=0.1, num_heads=None, scale=30, merge_pair=False):
        super().__init__()
        del num_heads
        self.num_hidden = num_hidden
        self.num_in = num_in
        ...
        self.merge_pair_embeddings = merge_pair

    def forward(self, h_E, h_EV, E_idx, mask_E=None, mask_attend=None, multiple_ensembles=False, k_neighbors=30):
        """ Parallel computation of full transformer layer """

        dh = self.W3(F.relu(self.W2(F.relu(self.W1(h_EV)))))
        if multiple_ensembles:
            # ensemble_splits = np.linspace(0, k_neighbors*8, 9, dtype=int) ## JFM
            dh_list = torch.split(dh, k_neighbors, dim=-2)
            h_E_list = torch.split(h_E, k_neighbors, dim=-2)
        else:
            # ensemble_splits = np.array([0, dh.shape[2]])
            dh_list = [dh]
            h_E_list = [h_E]
        h_E_list_final = []
        if mask_E is not None:
            mask_E = mask_E.unsqueeze(-1).unsqueeze(-1)
        for (dh, h_E) in zip(dh_list, h_E_list):
            dh = dh.contiguous()
            h_E = h_E.contiguous()
            if self.merge_pair_embeddings:
            ...
            if mask_E is not None:
                h_E = mask_E * h_E
            h_E_list_final.append(h_E)
        h_E = torch.cat(h_E_list_final, dim=-2)
        return h_E
