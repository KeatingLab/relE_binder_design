...
def merge_duplicate_edges(h_E_update, E_idx):
    ...
    """
    _,_,k = E_idx.shape
    test = torch.ones(h_E_update.shape[0]).long()
...
def merge_duplicate_pairE(h_E, E_idx, halve_pair_E=False):
...
def merge_duplicate_pairE_geometric(h_E, edge_index, halve_pair_E=False):
    ...
    if halve_pair_E:
        h_E[reverse_idx] = (h_E[reverse_idx] + transpose_h_E)/4
    else:
