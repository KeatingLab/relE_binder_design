...
class CondenseFlex(nn.Module):
    """ Flex Information Condensor

    Condense flexibility data to form a residue embedding

    Attributes
    ----------
    flex_type : str
        Indicates what flex data type is input

    n_hidden : int
        Size of latent space
    """

    def __init__(self, hparams, in_features=1, device='cuda:0'):
        """
        Args
        ----
        hparams : dict
        ...
        if self.hparams['flex_type'] == "conformational_ensembles_simple":
            if in_features==1:
                raise ValueError(f"{in_features} is not a valid feature number of input features for FlexCondense when input is conformational ensembles")
            layers = []

            for i in range(hparams['flex_layers']):
                out_features = hparams['flex_hidden_dim'][i]
                layers.append(nn.Linear(in_features, out_features))
                layers.append(nn.ReLU())
                p = hparams['flex_dropout'][i]
                layers.append(nn.Dropout(p))

                in_features = out_features
            layers.append(nn.Linear(in_features, self.hparams['flex_hidden_dim']))

            self.flex_condense = nn.Sequential(*layers)
        elif self.hparams['flex_type'] == "conformational_ensembles_attn":
            ## TODO
            x = 0
    ...
    def forward(self, data, max_seq_len):
        """ Convert input flex data into a latent representation
        ...
        agg_nodes : torch.Tensor
            Structure node embedding
            Shape: n_batch x max_seq_len x n_hidden
        agg_edges : torch.Tensor or None
            Structure edge embeddings
            Shape: n_batch x max_seq_len x max_seq_len x n_hidden
        """
        if self.hparams['flex_type'] == "dynamic_signature" and self.hparams['flex_hidden_dim'] == 1:
            return data['flex'], None
        elif self.hparams['flex_type'] == "conformational_ensembles_simple":
            # TO DO
            return self.flex_condense(data['flex'])
